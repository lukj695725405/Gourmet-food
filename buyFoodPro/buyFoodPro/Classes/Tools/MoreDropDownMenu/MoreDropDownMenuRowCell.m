...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        imageCount  = 3.0;
        imageSpace  = 20.0;
        imageWidth  = (ScreenW - imageSpace * 2 - (imageCount - 1) * imageSpace)/imageCount;
        imageHeight = 40.0;
    }
...
- (UIButton *)cellView:(NSString *)data tag:(NSInteger)tag{
    ...
    [bottomView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake((ScreenW - 2 * KImageMargin - 2 * KImageMargin)/KImageCount, KImageHeight * FIT_WIDTH));
    }];
